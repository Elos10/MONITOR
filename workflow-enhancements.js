...
loadRemoteState=async function(){
  ...
  try{
    ...
    const [usersSnap,eventsSnap]=await Promise.all([getDocs(collection(firebase.db,'users')),getDocs(collection(firebase.db,'events'))]);
...
function editObjectModal(id){
  ...
  const areas=['Gestão Pedagógica','Infraestrutura','Tecnologia','Logística','Gestão de Pessoas'],priorities=['Baixa','Média','Alta','Crítica'],statuses=['Planejado','Em andamento','Aguardando aprovação','Bloqueado','Concluído'];
